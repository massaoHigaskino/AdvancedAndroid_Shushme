...
public class MainActivity extends AppCompatActivity implements
    ...
    public void refreshPlacesData() {
        ...
        if(cursor == null || cursor.getCount() <= 0 || client == null) {
            return;
        }

        String[] idsArray = new String[cursor.getCount()];

        for(int i = 0; i < cursor.getCount(); i++) {
            idsArray[i] = cursor.getString(cursor.getColumnIndex(PlaceContract.PlaceEntry.COLUMN_PLACE_ID));
        }

        PendingResult<PlaceBuffer> placeById = Places.GeoDataApi.getPlaceById(client, idsArray);
