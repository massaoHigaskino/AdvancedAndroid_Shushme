...
public class MainActivity extends AppCompatActivity implements
    ...
    private static final int PLACE_PICKER_RESULT = 666;
    ...
    public void onAddPlaceButtonClicked(View view) {
        ...
        try {
            ...
            startActivityForResult(i, PLACE_PICKER_RESULT);
        } catch (Throwable e) {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if(requestCode == PLACE_PICKER_RESULT && resultCode == RESULT_OK) {
            Place place = PlacePicker.getPlace(this, data);

            ContentValues values = new ContentValues(1);
            values.put(PlaceContract.PlaceEntry.COLUMN_PLACE_ID, place.getId());

            getContentResolver().insert(Uri.parse(PlaceContract.AUTHORITY)
                    .buildUpon().appendPath(PlaceContract.PATH_PLACES).build(), values);
        } else {
